...
from letta.server.server import Server
...
def debug_actual_request():
    ...
    try:
        # 获取默认用户和智能体
        server = Server()
        user = server.get_default_user()
        print(f"✅ 获取默认用户: {user.id}")
        
        # 获取智能体
        agents = server.list_agents(user_id=user.id)
        agent = None
        for a in agents:
            if "personal-assistant" in a.name:
                agent = server.get_agent(agent_id=a.id)
                break
